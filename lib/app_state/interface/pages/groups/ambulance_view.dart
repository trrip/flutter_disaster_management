...
class AmbulanceView extends StatefulWidget {
  ...
  const AmbulanceView({Key key, @required this.user}) : super(key: key);
...
class _AmbulanceViewState extends State<AmbulanceView> {
  ...
  void navigateToPoint(LatLng point, int shouldAvoidDisaster) async {
    try {
      var response = await RemoteService().getRoute({
        ...
        "avoidDisaster": "$shouldAvoidDisaster",
        "group": "some",
      });
      ...
      floatingActionButton: Row(
        ...
        children: [
          ...
          RaisedButton(
            ...
            onPressed: () async {
              var response = await RemoteService().getNearestHospital({
                "lat": personMarker.point.longitude,
                "lon": personMarker.point.latitude
              });
              var first = response["Dest0"]["route"];
              this.setState(() {
                pathHolder = 1;
                navigationPath = getListOfCoordinates(first["coordinates"]);
                navigationMode = true;
              });
            },
          ),
